...
android {
    namespace = "io.solanatv.app"
    compileSdk = 34

    defaultConfig {
        applicationId = "io.solanatv.app"
        minSdk = 21
        targetSdk = 34
...
task("modifySource") {
    ...
    doLast {
        ...
        val url = when (net) {
            "ipv6" -> "DEFAULT_CONFIG_URL = \"https://freetv.fun/test_channels.m3u\""
            "mobile" -> "DEFAULT_CONFIG_URL = \"https://freetv.fun/test_channels.m3u\""
            else -> ""
        }

        if (url.isNotEmpty()) {
            val f = file("src/main/java/io/solanatv/app/SP.kt")
